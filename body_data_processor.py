...
def find_lower_devide_idx(data: List[Dict[str, Any]], lower_devide_feature: str) -> int | None:
...
def sort_by_y_mid(data: List[Dict[str, Any]], table_threshold_factor: float, lower_devide_feature: str) -> None | List[List[Dict[str, Any]]]:
    ...
    threshold: float = table_threshold_factor * float(average_height)
    ...
    for idx in range(lower_devide_idx+1, end_idx):
        ...
        if distance <= threshold:
            ...
            structured_body_data.append(current_row)
...
def get_features_x_mid_lines(data: List[Dict[str, Any]], features: List[str]) -> List[int | float] | None:
...
def get_closest_column_idxs(data: List[Dict[str, Any]], row: List[Dict[str, Any]], features: List[str]) -> List[int] | None:
    min_abs_distance_idxs = []
    feature_midlines: List[int | float] | None = get_features_x_mid_lines(data, features)
    if feature_midlines is None:
        return None

    for cell, feature_midline in zip(row, feature_midlines):
        cell_midline: int | float= (cell["box"][0] + cell["box"][2]) / 2  # x of the midline of the box
        abs_distances: List[int | float] = [abs(feature_midline - cell_midline) for feauture_midline in feature_midlines]  # the distances between the midline of the taget box and the standard midlines
        min_abs_distance_idxs.append(min(range(len(feature_midlines)), key=lambda x: abs_distances[x]))  # the index(s) of minimum of absolute value which infers to the column that the taget box belongs 
...
def append_cell_to_last_row(idxs: List[int], row: List[Dict[str, Any]], result_data, idx_to_key: Dict[int, str]) -> None:
...
def build_body_list(data: List[Dict[str, Any]], body_threshold_factor: float, devide_features: List[str]) -> List[Dict[str, str]] | None:
    result_data = []
    ...
    for row in structured_data:
        ...
        if len(row) < len(devide_features):
            ...
            min_abs_distance_idxs: List[int] | None = get_closest_column_idxs(data, row, devide_features)
        ...
        # Normal condition: number of elements equals to number of table columns
            ...
            result_data.append({idx_to_key[idx]: row[idx]["text"] for idx in range(len(idx_to_key))})
...
if __name__ == '__main__':
    settings = Settings()
    #Dummy code
    #build_body_list(original_data, structured_data, settings.DEVIDE_FEATURES, settings.TABLE_IDX_TO_KEY)
