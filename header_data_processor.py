...
def find_header_value_idx(
    ...
    header_key_idxs: List[int] | None = find_header_keys_idx(
        data,
        header_keys,
...
def get_header_pairs(
    ...
    tgt_strs = []
    value_idxs: List[int] | None = find_header_value_idx(
        data,
        header_keys,
        upper_devide_feature
    )
...
if __name__ == "__main__":
    settings = Settings()
    #build_header_dict(
        #data, settings.HEADER_KEYS,
        #settings.HEADER_THRESHOLD_FACTOR, settings.DEVIDE_FEATURES[0])
